...
class ActionNode:
    def __init__(self, name, value, parent, is_leaf):
        self.name = name  # leaf nodes: tha action string set, other nodes: particular string
        self.value = value  # root node: None   other nodes: choose probability
...
class PerturbationSelectionTree:
    def __init__(self, apk_info):
        ...
        self.android_root = ActionNode(name="root", value=None, parent=None, is_leaf=True)
    ...
    def build_code_subtree(self):
        # init layer --- code
        code_node = ActionNode(name="code", value=0.5, parent=None, is_leaf=True)  # The value can be set adaptively
    ...
    def build_service_layer(self):
        service_node = ActionNode(name="service", value=None, parent=None, is_leaf=True)
    ...
    def build_receiver_layer(self):
        receiver_node = ActionNode(name="receiver", value=None, parent=None, is_leaf=True)
    ...
    def build_provider_layer(self):
        provider_node = ActionNode(name="provider", value=None, parent=None, is_leaf=True)
    ...
    def generate_random_activity_name(self):
        activity_family_name = ''.join(random.sample(string.ascii_lowercase, 4))
        activity_package_name = ''.join(random.sample(string.ascii_lowercase, 4))
    ...
    def generate_random_receiver_name(self):
        receiver_family_name = ''.join(random.sample(string.ascii_lowercase, 5))
        receiver_package_name = ''.join(random.sample(string.ascii_lowercase, 5))
    ...
    # 计算子节点的概率权重=子节点的叶子节点数量/当前节点的叶子节点数量
    def build_middle_layer_probability(self, feature_node):
        total_nodes = sum([node.get_leaf_nodes_num_in_offspring() for node in feature_node.children])
    ...
    def build_uses_features_layer(self):
        uses_feature_node = ActionNode(name="uses-features", value=None, parent=None, is_leaf=True)
        hardwares = []
        softwares = []
    ...
    def build_hardware_layer(self, hardwares):
        hardware_node = ActionNode(name="hardware", value=None, parent=None, is_leaf=True)
        hardware_feature_layer = self.get_uses_feature_leaf_layer_list(hardwares, True)
        for name, value in hardware_feature_layer:
            hardware_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
    ...
    def build_software_layer(self, softwares):
        software_node = ActionNode(name="software", value=None, parent=None, is_leaf=True)
        software_feature_layer = self.get_uses_feature_leaf_layer_list(softwares, False)
        for name, value in software_feature_layer:
            software_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
        ...
        for group1, group2 in combinations(groups, 2):
            for permission_keywords1 in group1:
                for permission_keywords2 in group2:
                    common_keywords = len(permission_keywords1[1] & permission_keywords2[1])
    ...
    def build_permission_layer(self):
        permission_node = ActionNode(name="permission", value=None, parent=None, is_leaf=True)
        normal_permissions = []
    ...
    def build_normal_permission_layer(self, normal_permissions):
        normal_permission_node = ActionNode(name="normal_permission", value=None, parent=None, is_leaf=True)

        normal_permission_layer = self.get_leaf_layer_list_by_cluster(normal_permissions)

        for name, value in normal_permission_layer:
            normal_permission_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
            normal_permission_node.add_a_child(normal_permission_feature_node)
            self.validation_nodes.append(("permission", normal_permission_feature_node))
        return normal_permission_node

    def build_signature_permission_layer(self, signature_permissions):
        signature_permission_node = ActionNode(name="signature_permission", value=None, parent=None, is_leaf=True)
        ...
        for name, value in signature_permission_layer:
            signature_permission_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
            signature_permission_node.add_a_child(signature_permission_feature_node)
            self.validation_nodes.append(("permission", signature_permission_feature_node))
        return signature_permission_node

    def build_intent_layer(self):
        intent_node = ActionNode(name="intent", value=None, parent=None, is_leaf=True)
        activity_intents = []
        broadcast_intents = []
        category_intents = []
    ...
    def build_activity_intent_layer(self, activity_intents):
        activity_intent_node = ActionNode(name="activity_intent", value=None, parent=None, is_leaf=True)

        activity_intent_layer = self.get_leaf_layer_list_by_cluster(activity_intents)

        for name, value in activity_intent_layer:
            activity_intent_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
            activity_intent_node.add_a_child(activity_intent_feature_node)
            self.validation_nodes.append(("activity_intent", activity_intent_feature_node))
        return activity_intent_node

    def build_broadcast_intent_layer(self, broadcast_intents):
        broadcast_intent_node = ActionNode(name="broadcast_intent", value=None, parent=None, is_leaf=True)

        broadcast_intent_layer = self.get_leaf_layer_list_by_cluster(broadcast_intents)

        for name, value in broadcast_intent_layer:
            broadcast_intent_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
            broadcast_intent_node.add_a_child(broadcast_intent_feature_node)
            self.validation_nodes.append(("broadcast_intent", broadcast_intent_feature_node))
        return broadcast_intent_node

    def build_category_intent_layer(self, category_intents):
        category_intent_node = ActionNode(name="category_intent", value=None, parent=None, is_leaf=True)

        category_intent_layer = self.get_leaf_layer_list_by_cluster(category_intents)

        for name, value in category_intent_layer:
            category_intent_feature_node = ActionNode(name=name, value=value, parent=None, is_leaf=True)
            category_intent_node.add_a_child(category_intent_feature_node)
            self.validation_nodes.append(("intent_category", category_intent_feature_node))
        return category_intent_node
    
    # 从root到叶子节点的一条路径就是一个action
    def get_action(self):
