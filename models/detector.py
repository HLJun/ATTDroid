...
class Detector:
    def __init__(self, name, saving_path, feature, classifier):
        ...
        self.mlp_epochs = 1
        # 例如：saving_path/name_epochs or saving_path/name
    ...
    def build_classifier(self, dataset, save=True):
        ...
        # drebin和apigraph还需要对X进行向量化
        if self.feature == "drebin" or self.feature == "apigraph":
            if os.path.exists(self.saving_path + ".vec"):
                logging.debug(blue('Loading model from {}...'.format(self.saving_path + ".vec")))
        ...
        if os.path.exists(self.saving_path + ".clf"):
            logging.debug(blue('Loading model from {}...'.format(self.saving_path + ".clf")))
            ...
            if self.classifier == "svm":
                ...
                self.clf = MLP(input_dim=self.X_train[0].shape[-1], epochs=self.mlp_epochs)
            elif self.classifier == "rf":
                self.clf = RandomForestClassifier(max_depth=8, random_state=0)
            elif self.classifier == "fd_vae_mlp":
                self.clf = FD_VAE_MLP()
